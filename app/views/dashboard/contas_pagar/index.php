...
<?php if(PermissionService::has("api/contasapagar/insert")): ?>

		<div class="">
			<a onclick="inserirContaReceber()" class="btn btn-danger">
...
<?php component("/ContasPagar/Form", [
	'frequencias' => $frequencias,
	'formasPgto' => $formasPgto,
	'csrf' => $csrf,
]) ?>
<?php component("/ContasPagar/Baixar", [
	'formasPgto' => $formasPgto,
	'today' => $today,
	'csrf' => $csrf,
]) ?>
<?php component("/ContasPagar/Parcelar", [
	'formasPgto' => $formasPgto,
	'frequencias' => $frequencias,
	'today' => $today,
	'csrf' => $csrf,
...
<script>
	...
	const listarContasReceber = (data_ini='', data_fim ='', situacao = '')=>{
		$.ajax({
			url: `<?php echo route("/api/contasapagar") ?>?data_ini=${data_ini}&data_fim=${data_fim}&situacao=${situacao}`,
			method: "GET",
			dataType: "html",
			success: (result)=>{
				$("#listar").html(result);
	...
	const mostrar = ($user, action = '')=>{
		...
		$(document).ready(()=>{
			...
			$("#formContasReceber").attr("action", "../api/contasapagar/patch");
...
<?php if(PermissionService::has('api/contasapagar/patch')): ?>
<script>
...
<script>
	...
	const inserirContaReceber = ()=>{
		...
		$("#modalFormContasReceber").modal("show");
		$("#formContasReceber").attr("action", "/api/contasapagar/insert");
